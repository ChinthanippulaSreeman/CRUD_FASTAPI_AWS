...
def create_item(item: Item):
    ...
    """
    db_table = get_table()
    db_table.put_item(Item=item.model_dump())
    return item
...
# @app.get("/items/city/{city}", response_model=List[Item], tags=["Items"])
# def read_items_by_city(city: str):
#     """
#     Retrieve all items for a given city using the GSI.
#     """
#     db_table = get_table()
#     try:
...
#             detail=f"An unexpected error occurred while querying the database: {e.response['Error']['Message']}"
#         )
